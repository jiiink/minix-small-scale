...
static void getset_vtimer(struct mproc *mp, int which, const struct itimerval *value, struct itimerval *ovalue);
...
static clock_t ticks_from_timeval(const struct timeval *tv)
{
  ...
  if ((ticks / system_hz) != (unsigned long)tv->tv_sec) return LONG_MAX;
...
int do_itimer(void)
{
  ...
  bool setval, getval;
  int r, which = m_in.m_lc_pm_itimer.which;
  ...
  if (setval) {
    r = sys_datacopy(who_e, m_in.m_lc_pm_itimer.value,
                     PM_PROC_NR, (vir_bytes)&value, sizeof(value));
    if (r != OK) return r;
  ...
  if (r == OK && getval) {
    r = sys_datacopy(PM_PROC_NR, (vir_bytes)&ovalue,
                     who_e, m_in.m_lc_pm_itimer.ovalue, sizeof(ovalue));
  }
...
static void getset_vtimer(struct mproc *rmp, int which, const struct itimerval *value, struct itimerval *ovalue)
{
  ...
  r = sys_vtimer(rmp->mp_endpoint, num, value ? &newticks : NULL, ovalue ? &oldticks : NULL);
  if (r != OK) panic("sys_vtimer failed: %d", r);

  if (ovalue) {
    timeval_from_ticks(&ovalue->it_value, (oldticks <= 0) ? rmp->mp_interval[which] : oldticks);
  }
}
...
static void get_realtimer(struct mproc *rmp, struct itimerval *value)
{
  ...
  if (rmp->mp_flags & ALARM_ON) {
    remaining = tmr_exp_time(&rmp->mp_timer) - getticks();
...
static void set_realtimer(struct mproc *rmp, const struct itimerval *value)
{
  ...
  clock_t interval = (ticks <= 0) ? 0 : ticks_from_timeval(&value->it_interval);
...
static void cause_sigalrm(int arg)
{
  ...
  if ((rmp->mp_flags & (IN_USE | EXITING)) != IN_USE || !(rmp->mp_flags & ALARM_ON))
    return;
